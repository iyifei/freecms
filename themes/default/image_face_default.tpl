...
<body>
<h1>图片封面：{$freecms.typename}</h1>
<div class="clearfix">
    <div class="cleft">
        {channel id="arctype" pid="{$freecms.id}"}
            <div id="newslist" style="width:668px">
                <div class="title clearfix"><span><a href="{$arctype.typeurl}">{$arctype.id}
                            ,{$arctype.typename}</a></span><a
                ...
                <ul>
                    {arclist id="vo" key="k" typeid="{$arctype.id}" getall="1"  channel="{$arctype.channel}"}
                        <li>·<a href="{$vo.arcurl}" target="_blank"
                                ...
                                {$vo.title}</a><span>{$vo.pubtime|date_format:'%Y-%m-%d %H:%M:%S'}
                            </span>
                        </li>
                    {/arclist}
                </ul>
